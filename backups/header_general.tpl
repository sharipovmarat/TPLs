...
<!--	<div id="top_menu">
	...
	<div id="header">
		...
		<div>
			...
			{{/if}}
			<div id="main_menu">
				<a href="{{$config.project_url}}/" id="item1" title="Главная" {{if $smarty.server.SCRIPT_NAME=='/index.php'}}class="selected"{{/if}}></a>
				<a href="{{$config.project_url}}/latest-updates/" id="item2" title="Новые видео" {{if $smarty.server.SCRIPT_NAME=='/videos_list.php' && $list_videos_type=='latest'}}class="selected"{{/if}}></a>
				<a href="{{$config.project_url}}/top-rated/" id="item3" title="Самые рейтинговые" {{if $smarty.server.SCRIPT_NAME=='/videos_list.php' && $list_videos_type=='rating'}}class="selected"{{/if}}></a>
				<a href="{{$config.project_url}}/most-popular/" id="item4" title="Самые популярные" {{if $smarty.server.SCRIPT_NAME=='/videos_list.php' && $list_videos_type=='video_viewed'}}class="selected"{{/if}}></a>
				<a href="{{$config.project_url}}/categories/" id="item5" title="Категории" {{if $smarty.server.SCRIPT_NAME=='/categories.php'}}class="selected"{{/if}}></a>
				<a href="{{$config.project_url}}/community/" id="item6" title="Сообщество" {{if $smarty.server.SCRIPT_NAME=='/community.php'}}class="selected"{{/if}}></a>
				<a href="{{if $smarty.session.user_id>0}}{{$config.project_url}}/my_video_upload/{{else}}{{$config.project_url}}/login.php?action=not_allowed{{/if}}" id="item7" title="Загрузить видео"></a>
